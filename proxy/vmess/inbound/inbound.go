...
func (h *Handler) Process(ctx context.Context, network net.Network, connection internet.Connection, dispatcher routing.Dispatcher) error {
	...
	if sspanelruntime.ShouldReject(request.User.Email, connection.RemoteAddr().String()) {
		log.Record(&log.AccessMessage{
			From:   connection.RemoteAddr(),
			To:     "",
			Status: log.AccessRejected,
			Reason: "Rejected by SSPanel disconnect_ip",
			Email:  request.User.Email,
		})
		return newError("client rejected by SSPanel disconnect_ip: ", request.User.Email).AtInfo()
	}
	sspanelruntime.RecordAliveIP(request.User.Email, connection.RemoteAddr().String())
